...
export function ChemicalDetailCard({
  ...
  function renderField(col: ChemicalMasterColumn) {
    ...
    if (col.key === "tds_document") {
      const url = chemical.tds_document?.trim();
      return (
        <div className="mt-1 rounded-lg border border-slate-100 bg-slate-50 px-3 py-2.5">
          {url ? (
            <a
              href={url}
              target="_blank"
            ...
            <span className="text-sm text-slate-400 italic">No linked document</span>
          )}
          <p className="mt-1 text-[10px] text-slate-400">From linked TDS record</p>
        </div>
      );
    }
      ...
      <div className="space-y-6 p-6">
        ...
        {editingCell?.id === chemical.id ? (
          ...
          <p className="text-xs text-slate-400">
            Click any field to edit. Current price updates from Pricing &amp; Costing. TDS
            documentation links to the linked TDS PDF when available.
          </p>
        )}
      </div>
